  ...
  template<class type>
    ...
    public:
      ...
      camera( VOID ) :
        ...
        ProjDist(0.1), FarClip(100), Size(0.1),
        FrameW(30), FrameH(30)
      {
        UpdateProj();
        UpdateView();
       ...
       *   (ray<type>) result ray.
       */
      ray<type> FrameRay( const type &Sx, const type &Sy)
      {
        /* Obtain ray direction */
        vec3<type> A = Dir * ProjDist;
        vec3<type> B = Right * (Sx + 0.5 - FrameW / 2.0) * Wp / FrameW;
        vec3<type> C = Up * (FrameH / 2.0 - Sy - 0.5) * Hp / FrameH;
        vec3<type> Q = A + B + C;
